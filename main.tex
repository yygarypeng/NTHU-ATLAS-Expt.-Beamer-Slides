% Use 12 pt fonts and 16:9 frame
\documentclass[12pt,aspectratio=169]{beamer}
\usepackage{nthu-atlas}
...
\begin{frame}[fragile]{Selecting the Class} % if using "code" block, need to use [fragile].
    ...
    \begin{block}{Minimum SINTEF Beamer Document}
        % The end of lstlisting needs to be on the same last line of codes.
...
\date{Defaults to today's}\end{lstlisting}
    \end{block}
    \small{You can then write out the title page with \verb|\maketitle|.
...
% ...
\includegraphics
[width=\textwidth]{images/default}\end{lstlisting}
            \end{block}
        \end{column}
        \begin{column}{0.3\textwidth}
            \includegraphics[width=\textwidth]{source/nthu-name.png}\\
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}[fragile]
    \frametitle{Fonts}
    \begin{itemize}
        \item The paramount task of fonts is to be readable
...
\begin{frame}[fragile]{Look}
    \begin{itemize}
        \item To change the color of the title dash, give one of the class options
        ...
        themes for one slide because of the design of Beamer---and it's probably a
        good thing.
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \href{mailto:ruisong20@gmail.com}{send them to me!}
    \end{itemize}
\end{frame}
...
\begin{blank}
    \begin{frame}
        Here is an example of using the blank page test.
    \end{frame}
\end{blank}

\begin{frame}
\frametitle{Blank title recover test.}
    Is it back to normal? (Need to be yes.)
\end{frame}
